...
class PayPalService
{
    ...
    public function __construct()
    {
        ...
        // Kalo mau Paypal versi Live masukin code dibawah ini ya cuy
    ...
    {
        // Kalo mau ngasi shipping kasi value apapun
        $shipping = sprintf('%0.2f', 0);
        // Kalo mau ngasi pajak kasi value apa kek
        $tax = sprintf('%0.2f', 0);

        // Deklarasi variable pembeli
        $payer = new Payer();
        $payer->setPaymentMethod("paypal");

        // Masukin item item yang dibeli
        $items = array();
        ...
        // Ngasi keterangan pajak sama shipping
        $details = new Details();
        $details->setShipping($shipping)
            ->setTax($tax)
            ->setSubtotal(sprintf('%0.2f', $order->grand_total));

        // Ngasi jumlah yang harus dibayar
         $amount = new Amount();
